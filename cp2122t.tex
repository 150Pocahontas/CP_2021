...
Para grarantir que \ensuremath{\Varid{list2LTree}} não aceita listas vazias é pedido para definir \ensuremath{\Varid{outNEList}}.
...
Assim o resultado de \ensuremath{\Varid{list2LTree}} será uma árvore com o primeiro de cada sublista à esquerda
auxiliar

\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{splitHalf}\;(\Varid{a},\Varid{b})\mathrel{=}\Varid{splitAt}\;((\length \;\Varid{b}\mathbin{+}\mathrm{1})\mathbin{\Varid{`div`}}\mathrm{2})\;(\Varid{a}\mathbin{:}\Varid{b}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\>[17]{}(\Varid{id}+\Varid{splitHalf})\comp \Varid{outNEList}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
\end{minipage}}
\end{center}
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{getHash}\;(\Conid{Unit}\;\Varid{c})\mathrel{=}\p1\;\Varid{c}{}\<[E]%
\\
\>[B]{}\Varid{getHash}\;(\Conid{Comp}\;\Varid{a}\;\Varid{t})\mathrel{=}\Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{calcHash}\;(\Varid{t1},\Varid{t2})\mathrel{=}\Varid{concHash}\;(\Varid{getHash}\;\Varid{t1},\Varid{getHash}\;\Varid{t2}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks

Assim para obter o resultado pretendido usando o \ensuremath{\mathsf{in}} das \ensuremath{{\FTree}} que recebe o tipo:
...
\>[48]{}\Varid{id}}+\conj{\Varid{calcHash}}{\Varid{id}}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
\end{minipage}}
\end{center}
...
\vspace{0.5cm}
\par Pela análise do diagrama, concluimos que o gene \ensuremath{\Varid{g\char95 pairsList}} recebe uma lista e produz no,
...
\ensuremath{\Varid{getEvenBlock}}. Em seguida utilizamos uma vez o \ensuremath{\Varid{outList}}, olhando para o segundo elemento do either produzido por este,
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{g\char95 pairsList}\mathrel{=}(\Varid{id}+{}\<[23]%
\>[23]{}\Varid{assocl}\comp (\Varid{id}\times\conj{\Varid{head}}{\Varid{tail}}))\comp \Varid{outList}\comp \Varid{getEvenBlock}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
\end{minipage}}
\end{center}
...
o \ensuremath{\Varid{conquer}}, o que é um either, em que o lado esuqerdo é o \ensuremath{\Varid{head}} de uma lista,
que tendo em conta  o que este produz, será uma lista com uma \ensuremath{\Conid{MerkleTree}} singular,
isto é a \ensuremath{\Conid{Merkle}\;\Conid{Root}}. Conhecendo o tipo da \ensuremath{\Varid{joinMerkleTree}} foi possivel saber o tipo do
...
FTree \mathbb{Z}\ \mathbb{Z} ^+\ +\ FTree \mathbb{Z}\ \mathbb{Z} ^+ \times\ \mathbb{Z}^+\
		\ar[d]^{\ensuremath{\Varid{id}\times\Varid{id}\mathbin{+}\anaList{\Varid{divide}}}}
\\
...
FTree \mathbb{Z}\ \mathbb{Z} ^+\ +\ FTree \mathbb{Z}\ \mathbb{Z} ^+\ \times (FTree\ \mathbb{Z}\ \mathbb{Z} ^+)^+
		\ar[d]^{\ensuremath{\Varid{id}\times\Varid{id}\mathbin{+}\cataNat{\Varid{conquer}}}}
...
\vspace{0.5cm}
Distinto da primeira parte deste problema, em \ensuremath{{\FTree}\;\Varid{a}\;\Varid{c}}, c corresponde ao valor de hash de uma folha, da \ensuremath{\Conid{Unit}}, e o a
...
\par Para o divide primeiro aplicamos o \ensuremath{\Varid{outNEList}}, para ficar com um tipo idêntico ao pretendido, precisando agora apenas de
transpormar o elemento singular em \ensuremath{{\FTree}} e
...
\>[B]{}\Varid{classicMerkleTree}\mathrel{=}(\Varid{hyloNEList}\;\Varid{conquer}\;\Varid{divide})\comp (\map \;\Varid{\Conid{Main}.hash}){}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{singleFTree}\;(\Varid{a},\Varid{b})\mathrel{=}\Conid{Comp}\;(\Varid{concHash}\;(\Varid{a},\Varid{b}))\;(\Conid{Unit}\;\Varid{a},\Conid{Unit}\;\Varid{b}){}\<[E]%
\\
\>[B]{}\Varid{divide}\mathrel{=}\bot \;{}\<[21]%
\>[21]{}(\Varid{singleFTree}+)\comp \Varid{outNEList}\comp {}\<[55]%
\>[55]{}\Varid{pairsList}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{conquer}\mathrel{=}\bot \;\alt{\cdot }{\cdot }\;\Varid{head}\;\Varid{joinMerkleTree}\;\mathbf{where}{}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}\Varid{joinMerkleTree}\;(\Varid{l},\Varid{m})\mathrel{=}\Varid{mergeMerkleTree}\;\Varid{m}\;(\Varid{evenMerkleTreeList}\;\Varid{l}){}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}\Varid{mergeMerkleTree}\mathrel{=}\bot \;\llparenthesis\, \cdot \,\rrparenthesis\;\alt{h_1 }{h_2 }{}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}h_1 \;\Varid{c}\;\Varid{l}\mathrel{=}\Varid{c}{}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
\>[7]{}h_2 \;(\Varid{c},(\Varid{f},\Varid{g}))\;\Varid{l}\mathrel{=}(\Varid{c},((\Varid{head}\;\Varid{l}),\Varid{last}\;\Varid{l})){}\<[E]%
\\
\>[B]{}\hsindent{7}{}\<[7]%
...
\centerline{\xymatrix{
   (i, (X u i, X u i) \ar[rr]^-{\ensuremath{\Varid{assocl}}} && ((i,X u i),X u i) \ar[rr]^-{\ensuremath{\uncurry{\Conid{Node}}}} && X u (i, X u i) \ar[rr]^-{\ensuremath{\uncurry{\Conid{Node}}}} && X u i
}}
...
\\[\blanklineskip]%
\>[B]{}\Varid{hyloLTree}\;\Varid{f}\;\Varid{g}\mathrel{=}\Varid{cataX}\;\Varid{f}\comp \Varid{anaX}\;\Varid{g}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks

A evolução da biblioteca anterior para uma outra que permita partições n-arias (para qualquer n
finito) e nao apenas binárias
\begin{hscode}\SaveRestoreHook
\ColumnHook
\end{hscode}\resethooks
...
\vspace{0.3cm}
\subsubsection*{PairL}
\par Sendo o \ensuremath{\Varid{pairL}} um anamorfismo de listas podemos podemos representalo como o seguinte diagrama abaixo.
Deste retiramos que o gene de \ensuremath{\Varid{pairL}} produz, no caso de não vazia, um par à cabeça da mesma, com os dois primeiros elementos,
isto é, a cabeça e o primeiro elemento da cauda, e a cauda (\ensuremath{\Varid{tail}}).
\par Para obter esta solução, utilizamos o \ensuremath{\Varid{outList}} para produzir um either. Sendo que o lado esquerdo já está como queremos,
bastou, no lado direito, utilizar um split para por no tipo \ensuremath{\Conid{A}\times(\Conid{A}\times\Conid{A}\mathbin{*})}, em que o o segundo A é a \ensuremath{\Varid{head}} da \ensuremath{\Varid{tail}} da lista original,
...
&    1 + ((A,A)^* \times A^*)  \ar[d]^{\ensuremath{\Varid{recList}\;\Varid{pairL}}}
\\
    (A,A)^* &
     &  1 + ((A,A) \times (A,A)^*) \ar[ll]^-{\ensuremath{\Varid{inList}}}
}}
...
Chegando a estas conclusões podemos afirmar que, ao executar a \ensuremath{\Varid{toCell}} entre duas posições o resultado significa que:
\begin{itemize}
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{prop\char95 prefix2}\;\Varid{l}\;\Varid{l'}\mathrel{=}(\Varid{tail}\;\Varid{l'})\equiv (\Varid{tail}\;\Varid{l}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
\begin{propriedade}
...
\\
\>[B]{}\Varid{count}\mathrel{=}\overline{\Varid{unccount}}\;\mathbf{where}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{uncount}\;(\Varid{a},[\mskip1.5mu \mskip1.5mu])\mathrel{=}\mathrm{0}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{unccount}\;(\Varid{a},\Varid{l})\mathrel{=}\mathbf{if}\;(\Varid{a}\equiv \Varid{head}\;\Varid{l})\;\mathbf{then}\;\Varid{uncount}\;(\Varid{a},\Varid{tail}\;\Varid{l})\mathbin{+}\mathrm{1}\;\mathbf{else}\;\Varid{uncount}\;(\Varid{a},\Varid{tail}\;\Varid{l}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{inBounds}\;\Varid{m}\;(\Varid{x},\Varid{y})\mathrel{=}(\Varid{nlines}\;\Varid{m})\equiv \Varid{x}\mathrel{\wedge}(\Varid{ncols}\;\Varid{m})\equiv \Varid{y}{}\<[E]%
\\[\blanklineskip]%
